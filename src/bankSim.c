...
void initialize(queue *q) {
	q->cnt = 0; //müşteri sayısını ölçen counter
	q->front = q->rear = NULL;
}
int isEmpty(queue *q) { //boş mu kontrolü
	return (q->cnt == 0);
}
...
int dondur(int a) {
	int rastgele;
	rastgele = rand() % (4 + 1 - 1) + 1; //1-4 arasında işlem otomatik seçiliyor
...
int main() {
	...
	queue *q1 = (queue*) malloc(sizeof(queue)); //ilk çalışan
	initialize(q1); //ilk degerlere null atama islemi
	queue *q2 = (queue*) malloc(sizeof(queue)); //ikinci çalışan
	initialize(q2); //ilk degerlere null atama islemi
	queue *q3 = (queue*) malloc(sizeof(queue)); //üçüncü çalışan
	...
	while (zaman < N + 10) { //dongu en fazla N+10 defa donecek cogu zaman donguden erken ciktigi icin dinamik calisabiliyor.
		...
		d = rand() % (5 + 1 - 0) + 0;//musterinin gelmesi 0-5dk. . . aynı anda birden fazla musterinin gelmesi dahil degildir.
